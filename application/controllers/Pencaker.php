...
class Pencaker extends CI_Controller
{
    ...
    public function verifikasi_data() {
        // Mendapatkan data dari form atau request HTTP
        $id_user = $this->input->post("id_user");
        $status_verifikasi = $this->input->post("status_verifikasi");

        // Memanggil model untuk melakukan update id_status_cerifikasi
        $this->load->model('m_status_verifikasi');
        $result = $this->m_status_verifikasi->update_id_status_verifikasi($id_user, $status_verifikasi, $pesan);

        // Periksa hasil operasi update
        if ($result) {
            // Update berhasil
            $this->session->set_flashdata('update_success', 'input_status_verifikasi');
        } else {
            // Update gagal
            $this->session->set_flashdata('update_failed', 'eror_status_verifikasi');
        }

        // Redirect ke halaman tampilan atau halaman lain sesuai kebutuhan
        redirect('Pencaker/view_admin');
    }
    ...
    {
        $id_user = $this->input->post("id_user");
        $status_verifikasi = $this->input->post("id_status_verifikasi_option");
    ...
    public function aktif_data()
    {
        ...
        $result = $this->m_status_aktif->update_status_aktif_user($status_aktif, $id_user, $mulai_berlaku, $akhir_berlaku, $id_status_perpanjangan);
